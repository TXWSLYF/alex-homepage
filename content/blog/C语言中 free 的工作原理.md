...
对 C/C++ 来说，ASan 是定位 UAF/越界写的最强利器之一：

```bash
clang -fsanitize=address -fno-omit-frame-pointer -g demo.c -o demo
./demo
```

ASan 会用“红区（redzone）+ 影子内存（shadow memory）”把非法访问直接打爆，并给出堆栈。

### 9.2 Valgrind（Memcheck）

更通用但更慢，适合无法使用 sanitizer 的场景：

```bash
valgrind --leak-check=full ./demo
```

### 9.3 glibc malloc 相关环境变量（了解即可）

glibc 提供一些调试开关（不同版本行为略有差异），例如更严格的检查、填充特定字节等。线上不要随意开启，调试时可以配合文档使用。
