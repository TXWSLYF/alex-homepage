...
0) **快速返回**：`p == NULL` 直接 return。
1) **定位块元数据**：从 `p` 计算出 chunk/header 地址，读出 size、flags。
2) **一致性与安全检查（可能有）**：
   - size 合法性
   - 指针对齐（alignment）
...
4) **合并（coalescing）**：尝试与前后相邻空闲块合并，形成更大空闲块，减少碎片。
5) **（可选）归还给 OS**：当空闲块足够大、或位于堆顶（top chunk）时，分配器可能把一部分内存通过 `brk` 回收，或对 mmap 块做 `munmap`。
